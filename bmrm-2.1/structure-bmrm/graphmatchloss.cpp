...
void assignment(float* imFeatures, Scalar* w, float** indicator, int* classes, int maxviolator, int label, int** lossMatrix, float* preProduct, unsigned int* prePa)
{
  ...
  if (!pa) // 如果输入prePa为NULL, 则初始化pa为一个数组值从0~NCLASSES-1
  {
  ...
  if (!innerProducts) // 如果输入的之前的内积preProduct不等于NULL
  {
  ...
  else // 如果设定了maxviolator = 1
  {
    Scalar maxVal = -numeric_limits<Scalar>::max();
    Scalar sumPhi = numeric_limits<Scalar>::max();
    int* witnesses = new int [NNEIGHBOURS]; // witnesses用来存选取的y_\bar
    int firstInd = 0;
    for (int L = 0; L <= MAXLOSS; L ++) //对Loss的18个级别进行循环O(L)
    {
      if (L + sumPhi < maxVal) continue; // Since sumPhi decreases monotonically, this value of L cannot result in the best solution.
      // The inner-products are in decreasing order, so the ones used for L will be no bigger than the ones used for L-1.
      // 由于内积本身进行了降序排列， 因此用于L次循环的必定比L-1次循环小
      int ind = firstInd;
      int n = 0;
      while (n < NNEIGHBOURS and ind < NCLASSES) // 这个循环是对C个labels进行循环，然后选取NNEIGHBOURS个作为输出
      ...
      { // Find the minimum loss amongst the chosen labels. If the minimum loss is M, then we needn't consider solutions for L < M.
        ...
        if (lossMatrix[label][clazz] < minLoss) minLoss = lossMatrix[label][clazz]; // 每次给minLoss替换更小的， minLoss应该是min d(y, y^n)
      }
      if (minLoss < L) //这里的L对应于min d(c, y^n)
      {
        printf("Got impossible loss\n");
        exit(0);
      }
      L = minLoss;
      if (L + sumPhi > maxVal) // 如果找到了更大的max值，公式Eq.18， 则进行替换
      {
        maxVal = L + sumPhi;
        for (int n = 0; n < NNEIGHBOURS; n ++)
...
void assignment(float* imFeatures, Scalar* w, float** indicator, int* classes, int maxviolator, int* gtlabels, int** lossMatrix, float* preProduct, unsigned int* prePa)
{
	...
	float* innerProducts = preProduct;
	if (!innerProducts) // 如果?入的之前的内?preProduct不等于NULL
	{
		innerProducts = new float [NCLASSES];
		for (int i = 0; i < NCLASSES; i ++)
		{
		  imagePhi(imFeatures, indicator[i], phi); //imagePhi用来?算hardmard内?， 返回到phi中，phi是一个NFEATURE?度的向量
		  float innerProduct = 0;
		  if (w)
			for (int j = 0; j < NFEATURES; j ++) innerProduct += phi[j]*w[j];//?里是?算phi*w, 新的joint feature vector?
		  else
			for (int j = 0; j < NFEATURES; j ++) innerProduct += phi[j];
		  innerProducts[i] = innerProduct;
		}
	...
	if (!prePa) // 如果prePa == NULL
		MedianHybridQuickSort<float>(innerProducts, NCLASSES, pa); //?1000个classes的内??排序， 降序排列

	if (!maxviolator) // maxviolator = 0
	{
		for (int i = 0; i < NNEIGHBOURS; i ++)
		  classes[i] = pa[i]; //只取前5个，作?class的?出 (如果没有?定maxviolator)
	}
	else
	{
		Scalar maxVal = -numeric_limits<Scalar>::max();
		Scalar sumPhi = numeric_limits<Scalar>::max();
		int* witnesses = new int [NNEIGHBOURS]; // witnesses用来存?取的y_\bar
		...
		for(int c = 0; c < NCLASSES; c++)
		{
			...
			int ind = firstInd;
			int n = 0;
			while (n < NNEIGHBOURS and ind < NCLASSES) // ?个循?是?C个labels?行循?，然后?取NNEIGHBOURS个作??出
			{
				// caculate Ly
				for(int yn=0; yn < MAXLABELS; yn++)
				{
...
Scalar CGraphMatchLoss::LabelLoss(int* gtlabels, int* predlabels, int N){
	Scalar l = 0;
	Scalar loss = 0;
	for (int i = 0; i < MAXLABELS; i++){
		int gt_idx = *gtlabels[i];
		if(gt_idx == -1) break; // 遇到-1，表明gtlabel已经读取完成，跳出循环
		l = _data->lossMatrix[gt_idx][predlabels[0]];
		for (int j = 0; j < N; j++){
			if (_data->lossMatrix[gt_idx][classes[i]] < l) l = _data->lossMatrix[gt_idx][classes[i]];
		}
		loss += l;
	}
