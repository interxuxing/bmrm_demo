...
void CL2N2_BMRMDualInnerSolver::GetSolution(TheMatrix& w, double &objval)
{
   ...
   {
      int    w_dim    = w.Length();   
      int    last_dim = w_dim-positivity_dim;
      double val;
	 ...
	 if (val<0) val = 0; /* round-up to zero if necessary */
...
void CL2N2_BMRMDualInnerSolver::Solve(TheMatrix& w, TheMatrix& grad, double loss, double &objval)
{
   ...
   if (positivity_constraint)
   {
	...
	for (int j=0; j<dim; j++)
      ...
      int    last_dim = gradientSet[0]->Length()-positivity_dim;
      double grad;
      int    i1, i2;
	...
	for (int j=0; j<dim; j++)
	  ...
	  Q[i1+new_dim*i2] = Q[i2+new_dim*i1] = -grad*QPScale/bmrmLambda;
        }

      for (int i=0; i<positivity_dim; i++)
	for (int j=0; j<positivity_dim; j++)
	  Q[i+new_dim*j] = (i==j) ? QPScale/bmrmLambda : 0.0;
